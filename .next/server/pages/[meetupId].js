...
async function getStaticPaths() {
  const client = await external_mongodb_["MongoClient"].connect("mongodb+srv://Seo_abel:1234@cluster0.6v12y.mongodb.net/myFirstDatabase?retryWrites=true&w=majority");
  const db = client.db();
...
async function getStaticProps(context) {
  ...
  const client = await external_mongodb_["MongoClient"].connect("mongodb+srv://Seo_abel:1234@cluster0.6v12y.mongodb.net/meetups?retryWrites=true&w=majority");
  const db = client.db();
  const meetupsCollection = db.collection("meetups");
